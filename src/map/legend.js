...
//
// It binds its colour stops and mm range to the LIVE data from windowResult when
// present (docs/04 §3.3: raster.grid + stats), not to hardcoded values, and it
// swaps content by colourMode ('rainfall' | 'confidence' | 'spatialVariability',
// store.COLOUR_MODES). With no windowResult it shows an honest "no data" state.
//
...
/**
 * Derive the mm range to label from the window result, honouring the active
 * frame descriptor. Today only the accumulated window is available; per-frame
 * ranges plug in here when the scrubber arrives (the grid would carry per-frame
 * planes, or the adapter would hand a frame-scoped grid).
 *
 * @param {Object|null} windowResult
 * @param {{kind:'window'|'frame', index?:number, iso?:string}} frame
 * @returns {{ min:number|null, max:number|null }}
 */
function rangeFor(windowResult, frame) {
  if (!windowResult) return { min: null, max: null };
  // Prefer explicit grid metadata, then stats min/max cell. mmOf-style read.
  const grid = windowResult.raster && windowResult.raster.grid;
...
function rainfallHtml(range) {
  const haveRange = range.min != null && range.max != null;
  const rangeLabel = haveRange ? `${range.min.toFixed(1)} – ${range.max.toFixed(1)} mm` : 'no data yet';
  return `
...
function bodyFor(colourMode, windowResult, frame) {
  switch (colourMode) {
    case 'confidence': return confidenceHtml();
    case 'spatialVariability': return spatialHtml();
    case 'rainfall':
    default: return rainfallHtml(rangeFor(windowResult, frame));
  }
}
 ...
 * @param {L.Map} map
 * @returns {{
 *   update(args:{ colourMode:string, windowResult:Object|null,
...
export function createLegend(map) {
  ...
  let colourMode = 'rainfall';
  let windowResult = null;
  let frame = { kind: 'window' };
  let visible = true;
  ...
  function render() {
    if (!el) return;
    el.innerHTML = bodyFor(colourMode, windowResult, frame);
    el.style.display = visible ? '' : 'none';
  }

  function update(args = {}) {
    if (typeof args.colourMode === 'string') colourMode = args.colourMode;
    if ('windowResult' in args) windowResult = args.windowResult || null;
    if (args.frame) frame = args.frame; // reserved for the time-scrubber
    render();
  }
